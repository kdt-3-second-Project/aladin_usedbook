...
- 중고 책의 상태, 저자, 장르 등 다양한 요소들을 기반으로한 중고 책 판매 가격 예측
- 상품 별로 편차가 큰 중고 도서의 가격을 손쉽게 확인 가능한 정보를 위주로 하여 가격 예측

## 2. 목적

- 크롤링을 통해 알라딘 중고도서 데이터 셋 구축
- Random Forest Regressor, XGBoost 등의 모델을 이용한 알라딘 매장에서 공식적으로 판매하는 중고 도서 가격을 예측하는 회귀 모델 개발
...
- 알라딘의 중고 상품과 새 책 사이에 url 구조 등으로는 알라딘의 중고 도서와 새 책 사이에 구별할 수 없음
- 알라딘에서 도서별로 중고 상품을 정리해 놓은 페이지를 제공하는 것을 이용하여 데이터 셋을 체계적으로 구축
...
- [알라딘 온라인 중고매장(광활한 우주점)](https://www.aladin.co.kr/usedstore/wonline.aspx?start=we)에 등록 된 중고 도서로 데이터셋을 구성
...
- 총 1,415,586개의 row, column은 랭킹, 구분, 도서 명, ItemId, ISBN13, 부가기호, 저자, 출판사, 출판일, 정가, 판매가, 마일리지, 세일즈 포인트, 카테고리, 날짜로 12개
...
- 위의 베스트셀러 데이터에 포함된 서적(ItemId)을 기준으로 크롤링한 중고도서 매물 자료
...
- [도서 명](./research/240716_check_bookinfo.ipynb)
  ...
  - 로마 숫자를 아랍 숫자로 변환
  - 특수한 unicode 문자 detect하고, 흔히 쓰이는 특수문자로 변환
...
- train set에서 중고 시세를 학습한 적 없는 중고 매물에 대해서도 큰 차이 없는 성능으로 예측한 것으로 보아, NLP한 결과가 모델에 충분히 반영되었음을 알 수 있음
- 간단한 모델과 hyperparmeter로도 높은 성능이 나오는 것으로 보아, 알라딘 중고매장에서 중고 도서 판매 가격을 산정하는 가이드라인이 있을 것이라 추측 가능
